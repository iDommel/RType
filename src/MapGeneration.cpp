...
namespace ecs
{
    void GameSystem::createBoss(IScene &scene, Boss::BossType type, Position pos, QUuid id)
    {
        ...
        if (type == Boss::BossType::BOSS_1) {
            ...
            trajectory = std::make_shared<Trajectory>(std::function<float(float)>([](float a) { return -std::log(a); }),
                                                      std::function<float(float)>([](float a) { return 0; }),
                                                      position);

            boss->addMissileSalvo(Missile::MissileType::E_CLASSIC, 4000, 3, 200)
                .addMissileSalvo(Missile::MissileType::E_SINUSOIDAL, 10000, 10, 200)
                .addMissileSalvo(Missile::MissileType::E_HOMING_MISSILE, 20000, 1, 0);
        } else
            return;
