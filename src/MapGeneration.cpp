...
namespace ecs
{
    ...
    std::shared_ptr<Entity> GameSystem::whichEnemy(int mobId, int x, int y)
    {
        ...
        if (mobId == 1) {
            ...
            std::shared_ptr<Trajectory> trajectory = std::make_shared<Trajectory>(std::function<float(float)>([](float a) { return -a; }),
                                                                                  std::function<float(float)>([](float a) { return std::sin(a) * 2; }),
                                                                                  position);
